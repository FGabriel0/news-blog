...
const News = ({ onShowBlogs, blogs, onEditBlog, onDeleteBlog }) => {
 ...
 useEffect(() => {
  const fetchNews = async () => {
    try {
      let url = `https://gnews.io/api/v4/top-headlines?category=${selectedCategory}&lang=pt&country=br&apikey=e594a198a130f391ac23bccfbced3fa8`;

      if (searchQuery) {
        url = `https://gnews.io/api/v4/search?q=${searchQuery}&lang=pt&country=br&apikey=9c404c552f6102517c9c531e4d8475da`;
      }

      const response = await axios.get(url);
      const fetchedNews = response.data.articles;

      const newsWithImage = fetchedNews.map(article => ({
        ...article,
        image: article.image || noImg,
      }));

      setHeadline(newsWithImage[0]);
      setNews(newsWithImage.slice(1, 7));
